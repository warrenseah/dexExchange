...
contract('Dex', (accounts) => {
    ...
    it('should NOT deposit tokens if token does not exist', async () => {
        await expectRevert(
            dex.deposit(
                ...
                web3.utils.fromAscii('TOKEN-DOES-NOT-EXISTS'),
                {from: trader1}
            ),
    ...
    it('should NOT withdraw tokens if token does not exists', async () => {
        await expectRevert(
            dex.withdraw(
                web3.utils.toWei('100'),
                web3.utils.fromAscii('TOKEN-DOES-NOT-EXISTS'),
    ...
    it('should not create limit order is token balance is too low', async () => {
        await dex.deposit(
            web3.utils.toWei('99'),
            REP,
            {from: trader1}
    ...
    it('should not create limit order if dai token balance is too low', async () => {
        await dex.deposit(
            web3.utils.toWei('99'),
            DAI,
            {from: trader1}
